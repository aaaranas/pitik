...
export function RollDetail({ rollId }: { rollId: string }) {
  ...
  if (loading) {
    return <div className="h-full animate-pulse bg-cream-200" aria-hidden />;
  }
    ...
    return (
      <div className="grid h-full place-items-center bg-cream-50 px-6 text-center">
        <div>
          ...
          <Link href="/rolls" className="mt-5 inline-block text-sm text-sky-deep underline underline-offset-4">
            Back to your rolls
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto min-h-full w-full max-w-3xl bg-cream-50 pb-10">
      <header
        className="sticky top-0 z-20 flex items-center gap-1 bg-cream-50/90 px-2 pb-2 backdrop-blur"
      ...
      <div className="px-4">
        {renaming ? (
          ...
          <button
            ...
            className="text-left text-3xl font-semibold leading-tight tracking-tight text-cocoa-900"
        ...
        <>
          {strips.length ? (
            <div
              ...
              className="mt-6 flex gap-1 border-b border-cream-300 px-4"
            >
          ...
          {tab === "photos" ? (
            captures.length ? (
              <ContactSheet
                ...
                className="mt-4 border-t border-cream-300 pt-px"
              />
            ) : (
  ...
  return (
    <section className="mt-8 px-4">
      <div className="rounded-card border-2 border-dashed border-cream-300 px-6 py-10 text-center">
        ...
        <p className="mt-1 text-xs text-cocoa-600">Waiting is better, but it&rsquo;s your roll.</p>
      </div>
    </section>
  );
}
